...
[의학적 참고 지식]
1. 남성형 탈모증 (MPHL): 유전적 소인과 DHT에 의해 전두·정수리 모낭이 미세화됨. M자형 이마선 후퇴 및 정수리 탈모 패턴. Hamilton-Norwood 분류 사용.
2. 여성형 탈모증 (FPHL): 전두선은 보존되며 정수리 부위 모발만 점진적으로 가늘어짐(미세화). Ludwig 분류 사용.
3. 원형 탈모증 (AA): 자가면역질환으로 동심원 모양 탈모반, '느낌표 모발' 소견. SALT 점수 이용.
4. 휴지기 탈모증 (TE): 스트레스/영양부족 등으로 전두선 보존된 채 두피 전체에서 모발 가늘어짐 및 탈락.
5. 주요 임상 키워드: 모낭 소형화(Miniaturization), 연모화, 빈 모공, 두피 홍반/인설, 헤어라인 후퇴.

반드시 아래의 엄격한 JSON 형식으로만 응답해야 합니다.

[평가 기준 (Scoring Rubric - 임상 단계와 점수 매칭)]
사진의 탈모 심각도를 아래의 의학적 단계(Norwood, Ludwig, SALT)와 연동하여 엄격히 채점하세요.
- 80~100점 (양호/정상): 빽빽한 모발, 탈모 징후 없음. (Norwood I 이전 / 정상)
- 60~79점 (초기 진행): 가르마가 넓어지거나 이마선이 약간 후퇴. 연모화 시작. 
   -> [남성: Norwood II~IIIa] [여성: Ludwig I] [원형: SALT < 20% (경증)]
- 40~59점 (중기 진행): 두피가 확연히 들여다보임. 광범위한 연모화 및 밀도 감소. 
   -> [남성: Norwood III~IV] [여성: Ludwig II] [원형: SALT 20~50% (중등도)]
- 0~39점 (심각/말기): 광범위한 탈모, 빈 모공 다수, 두피 면적이 모발 면적을 압도함. 
   -> [남성: Norwood V~VII] [여성: Ludwig III] [원형: SALT ≥ 50% (중증)]
