    ...
    <span ref={rootRef} className={`depth-text ${className}`.trim()} style={rootStyle}>
      <span ref={stageRef} className="depth-text__stage">
        ...
        <span className="depth-text__face">
          {/* Injecting the gradient specifically for the 'Ai' part to match current branding */}
          <span className="bg-gradient-to-r from-teal-300 to-cyan-400 bg-clip-text text-transparent">Ai</span> 탈모분석
        </span>
      </span>
    </span>
  );
