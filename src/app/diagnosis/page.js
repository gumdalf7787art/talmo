    ...
    <div className={`flex flex-col relative ${isHistory ? 'gap-4 p-0' : 'gap-6 p-4'}`}>
      ...
      {result && (
        <div className={`flex flex-col gap-5 animate-in fade-in slide-in-from-bottom-4 duration-500 ${isHistory ? 'px-4' : ''}`}>
          ...
          {/* iOS Safari 등에서 flex/grid 렌더링 실패 방지를 위해 철저하게 절대 위치(position: absolute)로만 구성 */}
          <div 
          ...
          >
            ...
            <div style={{ position: 'absolute', top: '190px', left: '40px', width: '340px', height: '220px', backgroundColor: '#f8fafc', borderRadius: '24px', border: '3px solid #f1f5f9', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '70px', color: '#0f172a', fontWeight: '900' }}>{result.summary?.score || result.score || 0}<span style={{ fontSize: '32px', color: '#94a3b8' }}>/100</span></div>
            </div>
          ...
          <div className="w-full flex justify-end gap-2">
            ...
            <button 
              ...
              className="flex items-center gap-1.5 bg-[#FEE500] text-black px-3 py-2 rounded-lg font-bold text-[12px] shadow-sm hover:opacity-90 transition-all active:scale-[0.98]"
            >
              <Share2 className="w-3.5 h-3.5" /> 내 결과 카톡으로 공유하고 초대하기
            </button>
          </div>
