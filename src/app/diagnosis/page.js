...
function DiagnosisContent() {
  ...
  const handleShareResult = async () => {
    ...
    try {
      ...
      // 2. 800x800 정사각형 흰색 캔버스에 중앙 정렬하여 그리기 (카카오톡 1:1 크롭 방지)
      const img = new Image();
      img.src = imgData;
      ...
      // 이미지 비율 유지하면서 중앙에 맞추기
      const scale = Math.min(800 / img.width, 800 / img.height) * 0.9; // 10% 패딩
