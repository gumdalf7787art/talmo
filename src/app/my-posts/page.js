...
export default function MyPostsPage() {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      if (parsed.email) {
        fetch(`/api/posts/list?author=${encodeURIComponent(parsed.email)}`)
          .then(res => res.json())
          .then(data => {
            if (data.success) {
              setPosts(data.posts);
  ...
  if (isPC) {
    return <PCMyPosts posts={posts} isLoading={isLoading} />;
  }
    ...
    <div className="flex flex-col min-h-screen bg-gray-50 pb-safe">
      ...
      <main className="flex-1 p-4">
        <div className="flex items-center justify-between mb-4 px-1 text-[13px] text-gray-500">
          <span>총 <strong>{posts.length}</strong>건</span>
        </div>
        ...
        <div className="flex flex-col bg-white border-y border-gray-100 mt-2">
          {isLoading ? (
            ...
            <div className="py-10 text-center text-sm text-gray-500">작성한 게시글이 없습니다.</div>
          ) : (
            posts.map((post) => (
